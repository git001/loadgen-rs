...
usage() {
  cat <<'EOF'
Usage:
  scripts/h2_repeat_summary.sh --url <URL> [options] [-- <extra loadgen args>]

Runs repeated HTTP/2 benchmarks and prints per-run + average summary.
...
for ((i = 1; i <= REPEATS; i++)); do
  echo
  echo "=== h2 run ${i}/${REPEATS} (m=${MAX_STREAMS}) ==="
  out_file="$(mktemp)"
  cmd=(
  ...
  rm -f "$out_file"
  if [[ -z "$json_line" ]]; then
    echo "Could not find JSON result line (run=${i})" >&2
    exit 1
  fi
  jq -c --argjson round "$i" --argjson max_streams "$MAX_STREAMS" \
    '. + {round: $round, max_streams_actual: $max_streams}' <<<"$json_line" >>"$tmp_json"
done

echo
echo "=== Per-run summary ==="
jq -rs '
  ["round","m","rps","mbps_in","latency_p50_us","latency_p90_us","latency_p99_us","ttfb_p50_us","elapsed_s"],
  (.[] | [
    (.round|tostring),
...
jq -rs '
  ...
  | (["runs","m","rps_avg","rps_min","rps_max","mbps_in_avg","lat_p50_avg_us","lat_p90_avg_us","lat_p99_avg_us","ttfb_p50_avg_us","elapsed_avg_s"]),
    ([
      (.runs|tostring),
      (.m|tostring),
      (.rps_avg|tostring),
