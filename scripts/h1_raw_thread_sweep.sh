...
usage() {
  cat <<'EOF'
Usage:
  scripts/h1_raw_thread_sweep.sh --url <URL> [options] [-- <extra loadgen args>]

Runs HTTP/1.1 raw benchmark across thread counts.
...
IFS=',' read -r -a THREADS_VALUES <<<"$THREADS_LIST"

for threads in "${THREADS_VALUES[@]}"; do
  threads="$(echo "$threads" | xargs)"
  for ((i = 1; i <= REPEATS; i++)); do
    echo
    echo "=== raw threads=${threads} run ${i}/${REPEATS} ==="
    out_file="$(mktemp)"
    cmd=(
    ...
    rm -f "$out_file"
    if [[ -z "$json_line" ]]; then
      echo "Could not find JSON result line (threads=${threads}, round=${i})" >&2
      exit 1
    fi
    jq -c --argjson threads "$threads" --argjson round "$i" \
      '. + {threads_actual: $threads, round: $round}' <<<"$json_line" >>"$tmp_json"
  done
done
...
jq -rs '
  ["threads","round","rps","mbps_in","latency_p50_us","latency_p90_us","latency_p99_us","ttfb_p50_us","elapsed_s"],
  (.[] | [
    (.threads_actual|tostring),
    (.round|tostring),
    (.rps|tostring),
    (.mbps_in|tostring),
...
echo
echo "=== Thread averages ==="
jq -rs '
  group_by(.threads_actual)
  | map({
      threads: .[0].threads_actual,
      runs: length,
  ...
  | sort_by(.threads)
  | (["threads","runs","rps_avg","rps_min","rps_max","mbps_in_avg","lat_p50_avg_us","lat_p90_avg_us","lat_p99_avg_us","ttfb_p50_avg_us","elapsed_avg_s"]),
    (.[] | [
      (.threads|tostring),
      (.runs|tostring),
      (.rps_avg|tostring),
