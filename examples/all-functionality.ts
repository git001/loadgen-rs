...
function describe(name: string, fn: () => void): void {
  ...
  try {
    ...
    console.error(`[describe failed] ${name}: ${message}`);
  }
}

const chaiAny = chai as unknown as {
  config: Record<string, unknown>;
};
chaiAny.config.aggregateChecks = false;
chaiAny.config.logFailures = true;
...
function testSuite(): void {
  ...
  describe("docs example 6", () => {
    expect([1, 2]).to.have.ordered.members([1, 2]).but.not.have.ordered.members(
      [2, 1],
    );
  });

  describe("docs example 7", () => {
    expect([1, 2, 3]).to.include.ordered.members([1, 2]).but.not.include.ordered
      .members([2, 3]);
  });
  ...
  describe("docs example 112", () => {
    expect({ a: 1 }).to.have.ownPropertyDescriptor("a").that.has.property(
      "enumerable",
      true,
    );
  });
  ...
  describe("docs example 164", () => {
    ...
    expect([1, 2, 3]).to.have.members([2, 1, 3]).but.not.ordered.members([
      2,
      1,
      3,
    ]);
...
try {
  testSuite();
  console.log(
    `chai describe summary: total=${suiteStats.total} passed=${suiteStats.passed} failed=${suiteStats.failed}`,
  );

  const report = await bench.run() as RunReport;
  printK6LikeSummary(report, {
    scriptPath: "examples/all-functionality.ts",
    expectedProtocol: "h2",
  });
} finally {
  bench.close();
